...
// Reads the current page from props and drives the store's SET_PAGE.
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '11px 16px', borderTop: '1px solid var(--border)', flexWrap: 'wrap' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <button className="btn-table" style={{ opacity: currentPage >= tp ? .4 : 1, pointerEvents: currentPage >= tp ? 'none' : 'auto' }} onClick={() => dispatch({ type: 'SET_PAGE', payload: Math.min(tp, currentPage + 1) })}>Siguiente</button>
      </div>
    </div>
  );
}
